...
export default function Dashboard(props : HandleTab){
    ...
    return <Section cn='md:overflow-y-none overflow-y-auto md:w-5/6 w-full md:h-lvh h-5/6 flex flex-col md:flex-row justify-between bg-stone-200'>
        <Div cn='md:w-4/6 w-full md:order-1 mt-3 md:mt-0 order-2 h-lvh flex flex-col justify-around items-center'>
            <Div cn='w-[90%] md:w-5/6 bg-white shadow-xs shadow-black h-4/9 rounded-xl relative'>
                <Span cn="text-lg md:text-xl text-shadow-sm text-shadow-black text-lime-400 font-semibold absolute md:top-3 top-0 right-3 md:left-3 ">Attendance</Span>
                <Div cn="h-full w-full">
            ...
            <Div cn='w-[90%] md:w-5/6 bg-white shadow-xs shadow-black text-lime-400 h-4/9 rounded-xl'>
                <Div cn="flex items-center justify-between mb-4 h-1/4 px-4">
                    <h2 className="text-lg md:text-xl text-shadow-sm text-shadow-black text-lime-400 font-semibold">Recent Tests</h2>
                    <Button onClick={() => handleFunc('marks')} cn=" text-xs md:text-sm font-medium flex group items-center gap-1 text-shadow-xs text-shadow-black cursor-pointer hover:text-white">
                        View All
                        <Span cn='text-lime-400 group-hover:text-stone-400 md:block hidden'>
                            <FontAwesomeIcon icon={faGreaterThan}></FontAwesomeIcon>
                        </Span>
                    </Button>
                </Div>
                <Div cn="grid grid-cols-1 gap-4 h-2/4 overflow-y-auto">
                    {recentTests.map(test => (
                        <Div key={test.id} cn="border border-gray-100 rounded-lg p-4 hover:shadow-lg hover:shadow-black transition-shadow">
                            ...
                            <Div cn="mt-3">
                                ...
                                <Span cn="text-2xl font-bold text-lime-400 text-shadow-xs text-shadow-black">{test.score}</Span>
                                <Span cn="text-gray-500 text-shadow-xs text-shadow-black text-sm mb-1"> {test.total}</Span>
                                </Div>
                                <Div cn="w-full bg-gray-400 shadow-sm shadow-black rounded-full h-2 mt-2">
                                <div className="bg-lime-400 h-2 rounded-full" style={{ width: `${test.score}%` }} />
                                </Div>
                            </Div>
                        </Div>
                    ))}
        ...
        <Div cn='w-full md:w-2/6 md:order-2 order-1 h-lvh flex flex-col justify-center items-center'>
            <Div cn='h-full w-full flex flex-col justify-between items-center'>
                <Div cn='w-full h-1/3 flex justify-center items-center flex-col'>
                    <Span cn='bg-white h-3/5 block w-1/3 text-lime-400 text-shadow-sm text-shadow-black text-[4rem] rounded-full flex justify-center items-center'>
                    AK
                </Span>
                ...
                <Div cn='h-2/3 w-[90%] shadow-sm shadow-black rounded-xl md:rounded-t-xl md:rounded-b-none flex flex-col justify-start items-center bg-white'>
                    ...
                    <Div cn='h-5/6 w-full flex flex-col justify-start items-center divide-y-2 divide-lime-400/25 divide-opacity-25 px-3 overflow-y-auto'>
                        <Div cn='divide-x-2 divide-lime-400/25 w-full grid grid-cols-2 text-shadow-xs text-shadow-black py-2'>
                            <Span cn='text-lime-400 text-center'>Class</Span>
                            <Span cn='text-white text-center'>{studentData.class}</Span>
                        </Div>
                        <Div cn='divide-x- divide-lime-400/25 w-full grid grid-cols-2 text-shadow-xs text-shadow-black py-2'>
