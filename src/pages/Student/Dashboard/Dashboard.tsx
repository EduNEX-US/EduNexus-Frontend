...
export default function Dashboard(props : HandleTab){
    ...
    return <Section cn='md:overflow-y-none overflow-y-auto md:w-5/6 w-full md:h-lvh h-5/6 flex flex-col md:flex-row justify-between bg-white'> {/* DashBoard Section */}
        ...
        <Div cn='md:w-4/6 w-full md:order-1 mt-3 md:mt-0 order-2 h-lvh flex flex-col justify-around items-center'>
            {/* Attendance */}
            <Div cn='w-[90%] md:w-5/6 bg-black shadow-xs shadow-black h-4/9 rounded-xl relative'>
                {/* Title */}
                <Span cn="text-lg md:text-xl text-cyan-400 font-semibold absolute md:top-3 top-0 right-3 md:left-3 ">Attendance</Span>
                {/* Pie Chart */}
            ...
            {/* Recent Tests */}
            <Div cn='w-[90%] md:w-5/6 bg-black shadow-xs shadow-black text-cyan-300 h-4/9 rounded-xl'>
                {/* Title */}
                <Div cn="flex items-center justify-between mb-4 h-1/4 px-4">
                    <h2 className="text-lg md:text-xl text-cyan-400 font-semibold">Recent Tests</h2>
                ...
                <Div cn="flex flex-col justify-around items-center h-2/4 overflow-y-auto">
                    {recentTests.map(test => (
                        <Div key={test.id} cn="border border-gray-100 w-[90%] rounded-lg p-4 hover:shadow-lg hover:shadow-black transition-shadow mb-2">
        ...
        <Div cn='w-full md:w-2/6 md:order-2 order-1 h-lvh flex flex-col justify-center items-center'>
            <Div cn='h-full w-full flex flex-col justify-between items-center'>
                <Div cn='w-full h-1/3 flex justify-center items-center flex-col'>
                    <Span cn='bg-black h-3/5 block w-1/3 text-cyan-400 text-shadow-sm text-shadow-black text-[4rem] rounded-full flex justify-center items-center'>
                    AK
                </Span>
                <Span cn=' text-2xl font-bold text-center text-cyan-400 text-shadow-sm text-shadow-black'>Aman Kaushik</Span>
                </Div>
                <Div cn='h-2/3 w-[90%] shadow-sm shadow-black rounded-xl md:rounded-t-xl md:rounded-b-none flex flex-col justify-start items-center bg-black'>
                    <Span cn='font-extrabold text-xl h-1/6 py-1 text-white text-shadow-sm text-shadow-cyan-400'>Personal Details</Span>
