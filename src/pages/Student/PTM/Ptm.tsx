...
export default function Ptm(){
    return <Section cn='md:overflow-y-none overflow-y-auto md:w-5/6 w-full md:h-lvh h-5/6 flex flex-col justify-around items-center gap-4 bg-stone-200'> {/* PTM Section */}
      ...
      {ptmData.upcoming && (
        <Div cn="bg-gradient-to-r from-cyan-600 to-cyan-400 rounded-xl p-6 text-white w-[95%] md:w-3/4 ml-3 mt-4">
          ...
          <a href={ptmData.upcoming.meetLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-white text-cyan-400 font-medium rounded-lg hover:bg-indigo-50 transition-colors">
      ...
      <Div cn="bg-white rounded-xl p-6 shadow-sm border border-gray-100 w-[95%]  ml-3 md:w-3/4">
        ...
        <Div cn="space-y-4">
          {ptmData.past.map(meeting => (
            <Div key={meeting.id} cn="border border-gray-100 rounded-xl bg-black text-white p-5">
              ...
              <Div cn="bg-cyan-400 rounded-lg p-3"><p className="text-sm text-white"><Span cn="font-medium text-black">Summary: </Span>{meeting.summary}</p></Div>
            </Div>
          ))}
        </Div>
      </Div>
